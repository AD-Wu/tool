...
public class SqlServer extends Database {
    ...
    public int executeReader(IDataReader reader, String sql, Object[] args, int[] sqlTypes, int startIndex, int rows) throws Exception {
        String var7 = "SELECT * FROM (SELECT row_number()over(ORDER BY AX_TMP_COLUMN) AX_TMP_ROWNUM, * from (SELECT top " + (startIndex + rows) + " AX_TMP_COLUMN=0, AXT1.* from (" + sql + ") AXT1)) AXT2 WHERE AX_TMP_ROWNUM>=" + (startIndex + 1);
